...
!
      ...
      use vrbls2d, only: ths, qs, qvg, qv2m, tsnow, tg, smstav, smstot,       &
                         ...
                         AFWA_FZRA_HRLY, tsfcin,psfcin,t2min
        ...
        IF(MODELNAME .EQ. 'NCAR' .OR. MODELNAME.EQ.'RSM'.OR. MODELNAME.EQ.'RAPR')THEN
          DO J=JSTA,JEND
            DO I=1,IM
              ...
              PSFC(I,J)   = psfcin(i,j)      !PINT(I,J,NINT(LMH(I,J))+1)   !May not have been set above
...
!                  TSHLTR(I,J)=GRID1(I,J)
             ENDDO
           ENDDO
          print *,'2m th=',maxval(TSHLTR(1:im,jsta:jend)), &
             minval(TSHLTR(1:im,jsta:jend)),TSHLTR(1:3,jsta),'grd=',grid1(1:3,jsta)
        !print *,'2m tmp=',maxval(grid1(1:im,jsta:jend)), &
        !minval(grid1(1:im,jsta:jend)),grid1(1:3,jsta)
        !print *,'2m psfc=',maxval(PSHLTR(1:im,jsta:jend)), &
        !minval(PSHLTR(1:im,jsta:jend)),PSHLTR(1:3,jsta)
          if(grib=='grib1') then
            ID(1:25) = 0
            ISVALUE = 2
            ID(10) = MOD(ISVALUE/256,256)
