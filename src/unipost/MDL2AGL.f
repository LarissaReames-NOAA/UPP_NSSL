...
!
      ...
      use vrbls2d, only: refd_max, up_heli_max, up_heli_max16, grpl_max,      &
                         ...
                         up_heli_max0_05, up_heli_max01,                      &
                         up_heli_max0_15, up_heli_max05_15,                   &
                         up_heli_min0_05, up_heli_min01,                      &
                         up_heli_min0_15, up_heli_min05_15,                   &
                         ...
                         u10, v10, u10h, v10h, wind02max, spd02intmax
...
! KRF: Use arw/nmm output if thompson
        if (imp_physics==8 .or. imp_physics==28 .or. imp_physics==18) then
...
!---  Max Updraft Helicity 0-500 m
          IF((IGET(935).GT.0) )THEN
             DO J=JSTA,JEND
             DO I=1,IM
               GRID1(I,J)=UP_HELI_MAX0_05(I,J)
             ENDDO
             ENDDO
             ID(1:25)=0
             ID(02)=129
!             ID(11) = NINT(ZAGL(2))
             ...
             ENDIF
             if(grib=='grib1') then
               CALL GRIBIT(IGET(935),LP,GRID1,IM,JM)
             elseif(grib=='grib2') then
               cfld=cfld+1
               fld_info(cfld)%ifld=IAVBLFLD(IGET(935))
               fld_info(cfld)%lvl=LVLSXML(LP,IGET(935))
               if (IFHR  >  0) then
                 fld_info(cfld)%tinvstat = 1
               else
                 fld_info(cfld)%tinvstat = 0
...
!---  Max Updraft Helicity 0-1 km
          IF((IGET(936).GT.0) )THEN
             DO J=JSTA,JEND
             DO I=1,IM
               GRID1(I,J)=UP_HELI_MAX01(I,J)
             ENDDO
             ENDDO
             ID(1:25)=0
             ID(02)=129
!             ID(11) = NINT(ZAGL(2))
             ...
             ENDIF
             if(grib=='grib1') then
               CALL GRIBIT(IGET(936),LP,GRID1,IM,JM)
             elseif(grib=='grib2') then
               cfld=cfld+1
               fld_info(cfld)%ifld=IAVBLFLD(IGET(936))
               fld_info(cfld)%lvl=LVLSXML(LP,IGET(936))
               if (IFHR  >  0) then
                 fld_info(cfld)%tinvstat = 1
               else
                 fld_info(cfld)%tinvstat = 0
...
!---  Max Updraft Helicity 0-1.5 km
          IF((IGET(937).GT.0) )THEN
             DO J=JSTA,JEND
             DO I=1,IM
               GRID1(I,J)=UP_HELI_MAX0_15(I,J)
             ENDDO
             ENDDO
             ID(1:25)=0
             ID(02)=129
!             ID(11) = NINT(ZAGL(2))
             ...
             ENDIF
             if(grib=='grib1') then
               CALL GRIBIT(IGET(937),LP,GRID1,IM,JM)
             elseif(grib=='grib2') then
               cfld=cfld+1
               fld_info(cfld)%ifld=IAVBLFLD(IGET(937))
               fld_info(cfld)%lvl=LVLSXML(LP,IGET(937))
               if (IFHR  >  0) then
                 fld_info(cfld)%tinvstat = 1
               else
                 fld_info(cfld)%tinvstat = 0
...
!---  Max Updraft Helicity 0.5-1.5 km
          IF((IGET(938).GT.0) )THEN
             DO J=JSTA,JEND
             DO I=1,IM
               GRID1(I,J)=UP_HELI_MAX05_15(I,J)
             ENDDO
             ENDDO
             ID(1:25)=0
             ID(02)=129
!             ID(11) = NINT(ZAGL(2))
             ...
             ENDIF
             if(grib=='grib1') then
               CALL GRIBIT(IGET(938),LP,GRID1,IM,JM)
             elseif(grib=='grib2') then
               cfld=cfld+1
               fld_info(cfld)%ifld=IAVBLFLD(IGET(938))
               fld_info(cfld)%lvl=LVLSXML(LP,IGET(938))
               if (IFHR  >  0) then
                 fld_info(cfld)%tinvstat = 1
               else
                 fld_info(cfld)%tinvstat = 0
...
!---  Min Updraft Helicity 0-500 m
          IF((IGET(939).GT.0) )THEN
             DO J=JSTA,JEND
             DO I=1,IM
               GRID1(I,J)=UP_HELI_MIN0_05(I,J)
             ENDDO
             ENDDO
             ID(1:25)=0
             ID(02)=129
!             ID(11) = NINT(ZAGL(2))
             ...
             ENDIF
             if(grib=='grib1') then
               CALL GRIBIT(IGET(939),LP,GRID1,IM,JM)
             elseif(grib=='grib2') then
               cfld=cfld+1
               fld_info(cfld)%ifld=IAVBLFLD(IGET(939))
               fld_info(cfld)%lvl=LVLSXML(LP,IGET(939))
               if (IFHR  >  0) then
                 fld_info(cfld)%tinvstat = 1
               else
                 fld_info(cfld)%tinvstat = 0
...
!---  Min Updraft Helicity 0-1 km
          IF((IGET(940).GT.0) )THEN
             DO J=JSTA,JEND
             DO I=1,IM
               GRID1(I,J)=UP_HELI_MIN01(I,J)
             ENDDO
             ENDDO
             ID(1:25)=0
             ID(02)=129
!             ID(11) = NINT(ZAGL(2))
             ...
             ENDIF
             if(grib=='grib1') then
               CALL GRIBIT(IGET(940),LP,GRID1,IM,JM)
             elseif(grib=='grib2') then
               cfld=cfld+1
               fld_info(cfld)%ifld=IAVBLFLD(IGET(940))
               fld_info(cfld)%lvl=LVLSXML(LP,IGET(940))
               if (IFHR  >  0) then
                 fld_info(cfld)%tinvstat = 1
               else
                 fld_info(cfld)%tinvstat = 0
...
!---  Min Updraft Helicity 500-1500 m
          IF((IGET(941).GT.0) )THEN
             DO J=JSTA,JEND
             DO I=1,IM
               GRID1(I,J)=UP_HELI_MIN05_15(I,J)
             ENDDO
             ENDDO
             ID(1:25)=0
             ID(02)=129
!             ID(11) = NINT(ZAGL(2))
             ...
             ENDIF
             if(grib=='grib1') then
               CALL GRIBIT(IGET(941),LP,GRID1,IM,JM)
             elseif(grib=='grib2') then
               cfld=cfld+1
               fld_info(cfld)%ifld=IAVBLFLD(IGET(941))
               fld_info(cfld)%lvl=LVLSXML(LP,IGET(941))
               if (IFHR  >  0) then
                 fld_info(cfld)%tinvstat = 1
               else
                 fld_info(cfld)%tinvstat = 0
...
!---  Min Updraft Helicity 0-1500 m
          IF((IGET(942).GT.0) )THEN
             DO J=JSTA,JEND
             DO I=1,IM
               GRID1(I,J)=UP_HELI_MIN0_15(I,J)
             ENDDO
             ENDDO
             ID(1:25)=0
             ID(02)=129
!             ID(11) = NINT(ZAGL(2))
             ...
             ENDIF
             if(grib=='grib1') then
               CALL GRIBIT(IGET(942),LP,GRID1,IM,JM)
             elseif(grib=='grib2') then
               cfld=cfld+1
               fld_info(cfld)%ifld=IAVBLFLD(IGET(942))
               fld_info(cfld)%lvl=LVLSXML(LP,IGET(942))
               if (IFHR  >  0) then
                 fld_info(cfld)%tinvstat = 1
               else
                 fld_info(cfld)%tinvstat = 0
...
!---  Max Convective Wind 0-2 km
          IF((IGET(943).GT.0) )THEN
             DO J=JSTA,JEND
             DO I=1,IM
               GRID1(I,J)=WIND02MAX(I,J)
             ENDDO
             ENDDO
             ID(1:25)=0
             ID(02)=130
!             ID(11) = NINT(ZAGL(2))
             ...
             ENDIF
             if(grib=='grib1') then
               CALL GRIBIT(IGET(943),LP,GRID1,IM,JM)
             elseif(grib=='grib2') then
               cfld=cfld+1
               fld_info(cfld)%ifld=IAVBLFLD(IGET(943))
               fld_info(cfld)%lvl=LVLSXML(LP,IGET(943))
               if (IFHR  >  0) then
                 fld_info(cfld)%tinvstat = 1
               else
                 fld_info(cfld)%tinvstat = 0
...
!---  Max Integrated Convective Wind 0-2 km
          IF((IGET(944).GT.0) )THEN
             DO J=JSTA,JEND
             DO I=1,IM
               GRID1(I,J)=SPD02INTMAX(I,J)
             ENDDO
             ENDDO
             ID(1:25)=0
             ID(02)=130
!             ID(11) = NINT(ZAGL(2))
             ...
             ENDIF
             if(grib=='grib1') then
               CALL GRIBIT(IGET(944),LP,GRID1,IM,JM)
             elseif(grib=='grib2') then
               cfld=cfld+1
               fld_info(cfld)%ifld=IAVBLFLD(IGET(944))
               fld_info(cfld)%lvl=LVLSXML(LP,IGET(944))
               if (IFHR  >  0) then
                 fld_info(cfld)%tinvstat = 1
               else
                 fld_info(cfld)%tinvstat = 0
...
!---  Max Hail Diameter in Column (Thompson hail from GSD)
...
!---  Max Hail Diameter at k=1 (Thompson hail from GSD)
...
!
      ...
      SUBROUTINE REPLACE_BAD(ARR,IM,JM,JSTA,JEND)
         integer, intent(IN) :: IM, JM, JSTA, JEND
         real, intent(INOUT) :: ARR(IM,JM)

         logical, allocatable :: good(:,:)
         integer :: nbad, x_cur, y_cur, bad_cur, inds(2), i, j, jj, l
         real    :: min_dist
         integer, allocatable :: x_loc(:,:), y_loc(:,:), badx(:), bady(:)
         real, allocatable :: dist_mat(:,:)

         allocate(good(im,jend-jsta+1))
         nbad = COUNT(ISNAN(ARR(:,jsta:jend)))
         if (nbad .ge. 1) then
           allocate(badx(nbad),bady(nbad))
           allocate(dist_mat(im,jend-jsta+1), x_loc(im,jend-jsta+1), y_loc(im, jend-jsta+1))
           bad_cur = 1
           do jj = 1,jend-jsta+1
             j = jsta+jj-1
             do i=1,im
           ...
           do l = 1,nbad
             ...
             do jj=1,jend-jsta+1
               do i=1,im
                 dist_mat(i,jj) = SQRT( (float(x_loc(i,jj)) -float(x_cur))**2.0 + &
                           ( float(y_loc(i,jj)) -float(y_cur))**2.0 )
               enddo
             enddo
             min_dist = MINVAL(dist_mat,mask=good)
             inds = FINDLOC(dist_mat, min_dist,mask=good)
             ARR(x_cur,y_cur+jsta-1) = ARR(inds(1),inds(2)+jsta-1)
           enddo
           deallocate(badx, bady, dist_mat, x_loc, y_loc)
         endif
         deallocate(good)
      END SUBROUTINE REPLACE_BAD
